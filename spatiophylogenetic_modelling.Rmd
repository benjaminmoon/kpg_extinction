...
One of the features of many names is including a subspecies name in parentheses. While taxonomically important, this can be problematic as some taxa will include the subspecies, while others may not depending on the taxonomy. We thus remove the subspecies name. At this point, it's also useful to convert the strings to lowercase to remove any errors from capitalization.

```{r remove_subspecies}
bivalve_cleaned <-
  bivalve_cleaned |>
  mutate(tna_trimmed = str_replace_all(str_to_lower(tna), "\\(([:lower:]+)\\)\\s*", ""))
```

### Check spelling differences ###

Next, we check for spelling differences. Here, we use the *distance* between strings, in essence the number of characters different, to check whether there are misspelled names. Package *stringdist* has the function `stringdistmatrix` that calculates the pairwise distances within a string vector, thus showing the most similarly-spelled taxon names. Using an arbitrary distance cutoff — in this case 4 — will show the most similar strings.
...
str_indices <-
  tibble(
    pos_n       = which(str_dist_matrix <= 4 & str_dist_matrix > 0),
...
# bivalve_cleaned |>
#   mutate(tna_trimmed = map(name_corrections, str_replace_all, tna_trimmed, names(name_corrections))) |>
#   # filter(tna_trimmed == 'syncyclonema haggi') |>
#   pull(tna_trimmed)
```
...
errant_taxonomy |> pull(tna) |> unique()
```
...
The code below instead shows the lower taxa without a family specified. This are ripe for inclusion.
...
We find taxa that go extinct by charting those present before the K/Pg boundary that aren't present afterwards. Initially, do this for each species: we group into families later.
...
In the context of families, we can see the proportions of the families that go extinct.
...
fml_extinction_proportion <-
  ...
  left_join(species_extinctions, by = "tna") |>
  select(tna, fml, kpg_ext) |>

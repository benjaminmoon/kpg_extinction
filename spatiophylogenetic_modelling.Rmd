...
occ_outliers_taxon <-
  occ_checking %>%
  cf_outl(taxon = "tna", lat = "lat", lon = "lng", min_age = "lag", max_age = "eag", value = "flagged")
```
...
bivalve_pbdb_data <-
  bivalve_pbdb_data %>%
  mutate(
    cc_iso3 = countrycode(cc2, origin = "iso2c", destination = "iso3c", custom_match = cs_ma)
  )

bivalve_cleaned <-
  bivalve_pbdb_data %>%
  clean_fossils(taxon = "tna", min_age = "lag", max_age = "eag", lon = "lng", lat = "lat", countries = "cc_iso3", value = "clean") %>%
...
This finds `r nrow(errant_taxonomy)` records that wither have no family or `NA`. Most are identified to family level, but a similar number are species. It turns out that those with `NA` in the family are only identified to a higher taxonomic level, so we can leave them alone. 
...
errant_families |> pull(tna) |> unique()
```
